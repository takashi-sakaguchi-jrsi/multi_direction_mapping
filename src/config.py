...
class CameraConfig:
    ...
    def __post_init__(self):
        ...
        if self.lens_calibration_file:
            ...
            self.lens_calibration_file = None
...
@dataclass
class CaptureConfig:
    """側方撮影の実効視野・ROI設定（レンズintrinsicsとは分離）"""
    usable_outer_radius_ratio: float = 0.8
    derived_usable_half_fov_deg: float = 71.97
    nominal_sector_half_deg: float = 60.0
    required_margin_deg: float = 5.0
...
class RunConfig:
    ...
    physical_pitch_deg / physical_roll_deg はカメラカー定義（プログラム Euler とは別）。
    プログラム姿勢 [roll, yaw, pitch] は capture_to_program_orientation_deg で変換する。
    yaw_ref_deg は互換のため残すが、基準姿勢には使わない。
    """
    video_path: str = ""
    physical_pitch_deg: float = 90.0
    physical_roll_deg: float = 0.0
...
@dataclass
class CenterPriorConfig:
    """run_reference へのソフト拘束（係数は実データで調整）"""
    scale: float = 1.0
    sigma_x_mm: float = 1.0
    sigma_y_mm: float = 2.0
    sigma_roll_deg: float = 3.0
    sigma_yaw_deg: float = 5.0
    sigma_pitch_deg: float = 8.0


@dataclass
class HardBoundsConfig:
    """run_reference 中心のハードクリップ"""
    max_x_dev_mm: float = 5.0
    max_y_dev_mm: float = 10.0
    max_roll_dev_deg: float = 15.0
    max_yaw_dev_deg: float = 20.0
    max_pitch_dev_deg: float = 25.0


@dataclass
class RegistrationConfig:
    """2Run登録（RANSAC + 共同z補正）"""
    z_search_margin_mm: float = 50.0
    theta_search_margin_deg: float = 15.0
    ransac_iterations: int = 1000
    ransac_inlier_threshold_mm: float = 5.0
    min_inliers: int = 10
    z_model: str = "smooth"
    control_point_spacing_mm: float = 100.0
    lambda_ocr: float = 1.0
    lambda_smooth: float = 1.0
    max_correction_mm: float = 50.0
    max_local_scale_change: float = 0.2


@dataclass
class SeamWarpConfig:
    """接合面の 2 次元残差を半分ずつ戻す補正。端は固定しない。"""
    enabled: bool = True
    apply_edge_theta: bool = False
...
class BestViewConfig:
    """部分図蓄積。現行は座標上書き（d_wall 選択は使わない）。"""
    scoring: str = "overwrite"
    distance_tie_tolerance_mm: float = 1.0
    save_top_k: int = 1


@dataclass
class FeatureMatchingSideviewConfig:
    """壁面正対: レンズ中央の低歪矩形で抽出。空間格子間引きはしない。"""
    use_radius_filter: bool = False
    max_pixel_displacement_sideview: float = 250.0
    spatial_grid_rows: int = 8
    spatial_grid_cols: int = 8
    ratio_test_threshold: float = 0.75
    min_match_count: int = 10
    max_features: int = 2000
    center_rect_half_ratio: float = 0.4
    """レンズ半径 min(w,h)/2 に対する中央矩形の半辺。0.4 なら一辺は短辺の 0.4 倍。"""
    reject_reverse_travel: bool = True
    reverse_flow_tolerance_px: float = 1.0
    """フレーム y 増加（下向き）を逆走として落とすときの許容[px]。"""
    outlier_iqr_multiplier: float = 1.5
    """方向角・ベクトル長の IQR 外れ値倍率（オリジナルと同じ 1.5）。"""
    outlier_min_points: int = 4
    """IQR を掛ける最小点数。未満ならスキップ。"""


@dataclass
class ProjectionConfig:
    """側方パッチ投影。蓄積は逆投影＋双線形補間。sample_stride は順投影テスト用。"""
    allow_signed_relative_z: bool = True
    save_projection_cache: bool = False
    distance_metric: str = "coordinate"
    sample_stride: int = 2


@dataclass
class LegacyVpConfig:
    """PoC では常に false。既存正面経路のVP系列は残す"""
    enabled: bool = False
...
class TwoDirectionConfig:
    ...
    run_A: RunConfig = field(default_factory=lambda: RunConfig(
        run_id="U", physical_roll_deg=0.0
    ))
...
def _dataclass_from_dict(dc_cls, data: Optional[Dict[str, Any]]):
    """未知キーを無視して dataclass を構築する"""
    if not data:
        return dc_cls()
    names = {f.name for f in dc_cls.__dataclass_fields__.values()}
    return dc_cls(**{k: v for k, v in data.items() if k in names})


def build_two_direction_config(data: Optional[Dict[str, Any]]) -> TwoDirectionConfig:
    """two_direction セクションをネスト込みで構築する"""
    if not data:
    ...
    cfg.projection = _dataclass_from_dict(ProjectionConfig, data.get("projection"))
    cfg.legacy_vp = _dataclass_from_dict(LegacyVpConfig, data.get("legacy_vp"))
    return cfg
...
class Config:
    ...
    two_direction: TwoDirectionConfig = field(default_factory=TwoDirectionConfig)
    ...
    def from_dict(cls, config_dict: Dict[str, Any]) -> "Config":
        ...
        two_direction_cfg = build_two_direction_config(
            config_dict.get("two_direction")
        )
        ...
        config = cls(
            ...
            two_direction=two_direction_cfg,
        )
...
class ConfigValidator:
    ...
    def validate_all(self) -> None:
        ...
        self.validate_two_direction_config()
    ...
    def validate_two_direction_config(self) -> None:
        """2方向合成設定のバリデーション（動画パス空は許可）"""
        cfg = self.config.two_direction
        ...
        for run in (cfg.run_A, cfg.run_B):
            if run.physical_roll_deg not in (0.0, 120.0, 240.0, -120.0):
                raise ConfigValidationError(
                    ...
                    f"{run.physical_roll_deg}"
                )
        if cfg.capture.usable_outer_radius_ratio <= 0:
            raise ConfigValidationError(
                "capture.usable_outer_radius_ratio は正である必要があります"
            )
        if str(cfg.z_source).lower() not in ("ocr", "known"):
            raise ConfigValidationError(
                f"two_direction.z_source は ocr/known のみ: {cfg.z_source}"
            )
        ...
        if cfg.legacy_vp.enabled:
            self.logger.warning(
                "two_direction.legacy_vp.enabled=true ですが、"
                "PoC主経路ではVP系列を呼び出しません"
            )
