...
def test_capture_to_program_orientation():
    ...
    assert capture_to_program_orientation_deg(90.0, 0.0) == (0.0, 0.0, 90.0)
...
def test_run_reference_ids_and_program_angles():
    ...
    assert (u["roll_ref_deg"], u["yaw_ref_deg"], u["pitch_ref_deg"]) == (0.0, 0.0, 90.0)
...
def test_program_rotation_matches_demo_full_frame():
    ...
    for physical_roll in (0.0, 120.0, -120.0):
        ...
        np.testing.assert_allclose(R_prog.T, R_demo, atol=1e-6, err_msg=f"roll={physical_roll}")
